...
| `_truncate_tool_output()` | ツール出力が `max_tool_output_chars` を超える場合に切り詰め（コンテキスト膨張防止） |
...
Agent.plan(user_goal)
  ...
  ▼
CLIが計画を表示
  ├── 計画にツールが含まれる → ユーザーが全体承認 or キャンセル
  └── 計画にツールが含まれない → 承認スキップ（即実行）
  │
  ▼（実行する場合）
...
execute_react(goal, history, tool_hints=None)
  ...
  │              _truncate_tool_output(output)  ← max_tool_output_chars 超え時に切り詰め
  │              messages に tool(content=_wrap_tool_output(truncated)) を追加          │
...
| `max_tool_output_chars` 切り詰め | ツール出力 → messages 追加前 | 長大な出力を切り詰めてコンテキストの急膨張を防ぐ（デフォルト: 20000文字） |
| `context_limit × compress_threshold` | Memory.add() | 推定トークンが閾値を超えたら圧縮をトリガー |

**注意:** ReAct ループ内のコンテキスト（ツール結果の蓄積）は圧縮されない。`max_tool_output_chars` による切り詰めで急膨張を緩和しているが、ツールを多数呼ぶと `context_limit` に到達する可能性は残る。
...
max_tool_output_chars = 20000  # ツール出力の切り詰め上限（文字数）
