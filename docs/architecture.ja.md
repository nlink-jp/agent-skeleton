...
12. [POCとしての割り切りと将来課題](#12-pocとしての割り切りと将来課題)
...
│
├── tests/
│   ├── test_executor.py       ← _wrap_tool_output / execute_react テスト
│   ├── test_file_tool.py
│   ├── test_memory.py
│   ├── test_planner.py
...
| 3 | `<tool_call>` | ブロック全体を削除 | テキストモードで誤発火するLLM |
...
- LLMサマリーが正規化により空になった場合（インジェクション検知）はツール出力を直接表示 + `⚠` 警告
...
| `_build_result()` | 通常サマリー or インジェクション検知フォールバックの統一出力 |
| `_wrap_tool_output()` | プロンプトインジェクション対策フレーミング（§8.2参照） |
...
CLIが計画を表示 → ユーザーが全体承認 or キャンセル
  │
  ▼（承認された場合）
Agent.execute(user_goal, plan)           ← plan は表示用。実行には使わない
  ├── Memory.get_messages()
  ├── Executor.execute_react(goal, history)   ← §5.3 へ
...
```
execute_react(goal, history)
  │
  messages = [*system_msgs, user(goal)]
  │
...
モデルがインジェクションに操作されて内部マークアップを出力した場合、`_normalise_content` がそれを除去してコンテンツが空になる。この場合:

1. `⚠ 注意: ...プロンプトインジェクションの可能性...` をユーザーに表示
2. ツールの生出力（攻撃テキストではなく、ツールが返したデータ）をフォールバック表示
3. ログに `WARNING` を記録

これにより「インジェクションで操作されたが、元のタスク（ファイル内容の表示など）は遂行できた」という状態になる。
